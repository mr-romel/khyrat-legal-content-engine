...
# Existing topic-rotation compatibility patch.
try:
    import hashlib
    ...
    def _patched_topic_pool_for_month(current_key, used_topics):
        ...
        available = [item for item in _rules.EXPANDED_TOPIC_BANK if _rules.normalize_topic(item["topic"]) not in used]
        if not available:
        ...
        ordered_categories = [_rules.CATEGORY_ORDER[(start + i) % len(_rules.CATEGORY_ORDER)] for i in range(len(_rules.CATEGORY_ORDER))]
        ...
        brief_keys = {( _rules.normalize_topic(item["topic"]), item.get("category", "").strip().casefold(), item.get("angle", "").strip().casefold()) for item in result}
...
# New transient-Gemini resilience patch. It retries only service/rate-limit/
# timeout failures and never retries arbitrary application errors.
try:
    from google import genai
    from gemini_resilience import patch_client

    _OriginalClient = genai.Client

    class _ResilientClient(_OriginalClient):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            patch_client(self)

    genai.Client = _ResilientClient
except Exception as exc:
    print(f"Gemini resilience shim not activated: {exc}", flush=True)
