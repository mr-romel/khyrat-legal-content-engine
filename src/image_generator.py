...
class ImageGenerationError(RuntimeError):
    """Raised when image generation or finalization fails."""
...
def _extract_image_bytes(response: requests.Response) -> bytes:
    ...
    encoded = result.get("image") if isinstance(result, dict) else result if isinstance(result, str) else None
    if not encoded:
...
Do not create a generic legal image. Depict the actual story.
""".strip()
    return prompt[:MAX_PROMPT_LENGTH].rsplit(" ", 1)[0].strip() if len(prompt) > MAX_PROMPT_LENGTH else prompt


def _finalize(image_bytes: bytes, output: Path, page_name: str | None) -> None:
    try:
        image = Image.open(BytesIO(image_bytes)).convert("RGB")
    except Exception as exc:
        raise ImageGenerationError(f"Could not decode generated image: {exc}") from exc
    final = ImageOps.fit(image, (1024, 1280), method=Image.Resampling.LANCZOS, centering=(0.5, 0.43))
    final = add_page_branding(final, page_name)
    output.parent.mkdir(parents=True, exist_ok=True)
    final.save(output, format="JPEG", quality=94, optimize=True)


def create_legal_image(*, topic: str, image_brief: str, output_path: str,
                       cloudflare_account_id: str | None = None,
                       cloudflare_api_token: str | None = None,
                       page_name: str | None = None) -> str:
    ...
    if not response.ok:
        ...
        raise ImageGenerationError(f"Cloudflare image API failed: HTTP {response.status_code} - {detail}")
    image_bytes = _extract_image_bytes(response)
    if not image_bytes:
        raise ImageGenerationError("Cloudflare returned empty image bytes.")
    output = Path(output_path)
    _finalize(image_bytes, output, page_name)
    ...
    print(f"Page branding applied successfully: {page_name or 'اسأل محمود'}")
    return str(output)
