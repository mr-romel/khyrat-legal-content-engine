...
from src.recycler_rules import (
    RECYCLE_MARKER, POSTING_TIMES, ANGLE_LIBRARY,
    normalize_topic, historically_used_topics, source_rows,
    prepared_slots, posting_days, topic_pool_for_month,
...
def _topic_pool_for_month(month_key: str, used_topics: set[str]):
    """Backward-compatible alias for legacy tests and callers."""
    return topic_pool_for_month(month_key, used_topics)
...
def recycle_month_if_needed(*, service, spreadsheet_id: str, sheet_name: str, current: date) -> int:
    ...
    replaced = replace_remaining_current_month_slots(service, spreadsheet_id, sheet_name, values, current, current_key, pool)
    if replaced:
        values = get_values(service, spreadsheet_id, f"{sheet_name}!A:U")
        historical_used = historically_used_topics(values, current_key)
        pool = topic_pool_for_month(current_key, historical_used)
    days = posting_days(current.year, current.month, current.day)
    expected = {(f"{current.year:04d}-{current.month:02d}-{day:02d}", t) for day in days for t in POSTING_TIMES}
    ...
    for index, (publish_date, posting_time) in enumerate(missing):
        ...
        if brief is None:
            ...
            row = {
                ...
                "الموضوع": f"{original} — زاوية جديدة: {angle}", "تاريخ النشر": publish_date,
                "ساعة النشر": posting_time, "نوع الجدولة": "DATE_TIME", "الحالة": "READY",
            ...
            row = {
                "ID": f"{publish_date.replace('-', '')}-{posting_time.replace(':', '')}-R{index + 1:03d}",
                "الموضوع": f"{original} — زاوية جديدة: {brief['angle'].strip()}", "تاريخ النشر": publish_date,
                "ساعة النشر": posting_time, "نوع الجدولة": "DATE_TIME", "الحالة": "READY",
