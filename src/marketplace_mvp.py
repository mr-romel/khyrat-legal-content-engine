...
def build_offer(opportunity: dict[str, Any]) -> str:
    return ("مرحباً، قرأت تفاصيل المشروع وأرى أن المطلوب يتوافق مباشرة مع خبرتي في الأعمال القانونية، خصوصاً تحليل المخاطر وصياغة ومراجعة المستندات.\n\n"
            "سأبدأ بفهم المستند أو المتطلبات وتحديد النقاط القانونية المؤثرة، ثم أقدم الملاحظات والصياغات المقترحة بشكل واضح ومنظم، مع الالتزام بنطاق العمل المتفق عليه.\n\n"
            f"المدة المقترحة: {opportunity.get('suggested_days', 3)} أيام. والميزانية المقترحة: {opportunity.get('suggested_price_egp', 1500)} جنيه مصري.\n\n"
